...
function customSet(arr) {
  ...
  for (const element of setNew) {
    if (typeof element === "number") {
      setNew.delete(element);
    }
  }
  return setNew;
...
function setToArray(mySet) {
  const arr = [...mySet];
  const filteredArrWithString = arr.filter(
    (value, index, array) => typeof value === "string"
  );
  return filteredArrWithString.sort();
...
function areDisjoint(set1, set2) {
  ...
  return true;
  // return set2.has(element) ? "false" : "true";
...
function getDifference(set1, set2) {
  ...
  for (const element of set1) {
    !set2.has(element) ? differenceSet.add(element) : "";
  }
...
function iterateSet(set) {
  ...
  const entri = set.entries();
  for (const element of entri) {
    console.log(element);
  }
